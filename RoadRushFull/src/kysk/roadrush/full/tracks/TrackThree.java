package kysk.roadrush.full.tracks;

import kysk.roadrush.full.Constants;
import kysk.roadrush.full.GameView;
import kysk.roadrush.full.OnGameOverListener;
import kysk.roadrush.full.R;
import kysk.roadrush.full.Summary;
...
public class TrackThree extends Activity implements OnGameOverListener, OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		best = prefs.getInt(Constants.PREFS_TRACKTHREE_BEST, 0);
		...
		gameView = new GameView(this, soundManager, Constants.TRACK_THREE);
	...
	public void onGameOver(int score, int swipes, long elapsedTime) {
		...
		editor = prefs.edit();
		if (score > best) {
			editor.putInt(Constants.PREFS_TRACKTHREE_BEST, score);
		}
		...
		editor.commit();
		Intent summary = new Intent(TrackThree.this, Summary.class);
		summary.putExtra(Constants.EXTRA_TRACK, Constants.TRACK_THREE);
